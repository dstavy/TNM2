...
//#defne USE_MOVIE
const string PresentationUpdate::JSON_FILE_LOCAL_BKP = "records/dataset_bkp.json";
	...
	if (file.doesFileExist(json)) {
		...
		if (t > lastUpdate) {
			...
			if (datasetJson.open(json)) {
				// create bkp
				file.copyTo(JSON_FILE_LOCAL_BKP, true, true);
			   ...
			   // profileTracker->setThreaded(false);
				...
				// copy from bkp
				file.copyFromTo(JSON_FILE_LOCAL_BKP, JSON_FILE_LOCAL, true, true);
			}
		}
		else {
			// file not udpated!

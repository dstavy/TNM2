//
//  Globals.h
//  TNM2
//
//  Created by inx on 13.11.18.
//

#ifndef Globals_h
#define Globals_h


// turn on watchdog - windows only
//#define DO_WATCHDOG


// draws some additional information
//#define DRAW_DEBUG


//#define DO_BLACK_FRAME
//#define DO_BLACK_FRAME_ADJUST

//#define NO_RELEASE_BERLIN

// settings
#define SCREEN_WIDTH 1200	//
#define SCREEN_HEIGHT 1920
#define MUGSHOT_IS_LEFT
//#define GRID_SCALE_TABLE 1.0//0.37125f  // scaling factor for grid-image
//#define GRID_SCALE 1.0//0.37125f//0.66 * (1080.0/1920.0)
#define TIME_WAIT_BEFORE_FLY 1.0
#define TIME_FEATURE_SCALE 0.5
#define TIME_FEATURE_FLY 1.5
#define TIME_ROW_STEP 0.05
#define TIME_GIRD_MOVE_FEATURE 1.0
#define TIME_GIRD_MOVE_IDLE 1.5
#define TIME_GIRD_MOVE_DELAY 0.1

#define FEATURE_SCALE 1.2

#endif /* Globals_h */

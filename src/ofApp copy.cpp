...
void ofApp::update(){
    ...
    if(player->isFrameNew()){
        ...
        if(tracker->size()){
            ...
            ofRectangle allRec = camFace.getBoundingBox();
...
void ofApp::draw(){
    ...
    if (select == 0) {
        if (tracker0.size()) {
        drawVideo(frontPlayer, 100, 100, 600, 600);
        }
    }
   else {
       if (tracker1.size()) {
        drawVideo(sidePlayer, 100, 1000, 600, 600);
       }
    }
    ...
    */
    ...
    ofDrawBitmapStringHighlight("Tracker thread framerate : "+ofToString(tracker0.getThreadFps()), 10, 40);
...
void ofApp::drawVideo(ofVideoPlayer& player, int x, int y, int w, int h) {
   ...
   player.getTexture().drawSubsection(x, y, w, h, boundBoxes[6].x - 0.5* (w - boundBoxes[6].width),boundBoxes[6].y -  0.25* (h - boundBoxes[6].height), w, h);
        if ((ofGetElapsedTimeMillis() -  lastGridUpdate) > timeElapse) {
         ...
         //   ofTranslate(1000, 100);
          ...
          // path.draw();
           // ofDisableAlphaBlending();
            ...
            player.getTexture().drawSubsection(p.x, p.y, gridSize/3, gridSize/3, boundBoxes[6].x - 0.5*(boundBoxes[6].height- boundBoxes[6].width), boundBoxes[6].y , boundBoxes[6].height, boundBoxes[6].height);
           gridFbo.end();
...
void ofApp::keyReleased(int key){
   /* if (key == 'r'){
        ...
        rotation -= 90;
        rotation = rotation % 360;
        tracker0.setFaceRotation(rotation);
        tracker1.setFaceRotation(rotation);
    } /*else if (key == 'c'){
